...
async function getdata(links) {
    const response = await fetch(`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${links}`);
...
jshareBtn.addEventListener("click",async()=>{
    try {
        ...
        const sharefile = new File([datashare],'qrcode.png',{type: "image/png"});
